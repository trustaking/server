...
NETWORK=""
NODE_USER=x42${NETWORK}
COINCORE=/home/${NODE_USER}/.x42node/x42/X42Main
COINPORT=52342
COINRPCPORT=52343
COINAPIPORT=42220
DNSPORT=53
...
NETWORK="-testnet"
NODE_USER=x42${NETWORK}
COINCORE=/home/${NODE_USER}/.x42node/x42/X42Test
COINPORT=62342
COINRPCPORT=62343
COINAPIPORT=42221
DNSPORT=53
...
if [ "${NETWORK}" = "" ] ; then
   DNS="-iprangefiltering=0 -externalip=${NODE_IP} -dnshostname=seed.x42coin.com -dnsnameserver=dns1.seed.x42coin.com -dnsmailbox=admin@x42platform.com -dnsfullnode=1 -dnslistenport=${DNSPORT}"
 else
   DNS="-iprangefiltering=0 -externalip=${NODE_IP} -dnshostname=seed.x42platform.com -dnsnameserver=testdns1.seed.x42platform.com -dnsmailbox=admin@x42platform.com -dnsfullnode=1 -dnslistenport=${DNSPORT}"
fi
...
COINRUNCMD="sudo dotnet ./x42.x42D.dll ${NETWORK} -datadir=/home/${NODE_USER}/.x42node -iprangefiltering=0 ${DNS}"  ## additional commands can be used here e.g. -testnet or -stake=1
CONF=release
COINGITHUB=https://github.com/x42protocol/X42-FullNode-UI.git
COINDAEMON=x42d
COINCONFIG=x42.conf
COINSTARTUP=/home/${NODE_USER}/x42d
COINDLOC=/home/${NODE_USER}/x42Node
...
echo && echo
echo -e "${PURPLE}**********************************************************************${NONE}"
echo -e "${PURPLE}*    This script will install and configure your x42 node.      *${NONE}"
echo -e "${PURPLE}**********************************************************************${NONE}"
echo -e "${BOLD}"
...
echo -e "${BOLD}"
read -p " Do you want to setup on Mainnet (m), Testnet (t) or upgrade (u) your x42 node. (m/t/u)?" response

if [[ "$response" =~ ^([mM])+$ ]]; then
    setMainVars
    read -p " Do you want to setup your x42 node as a DNS Server (y/n)?" response
    echo -e "${NONE}"
    if [[ "$response" =~ ^([yY])+$ ]]; then
        setDNSVars
    fi
